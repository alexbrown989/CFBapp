"""Validate the unexecuted N03 notebook and, when present, its deliverables."""
...
NB_PATH = pathlib.Path(__file__).resolve().parent / "03_walk_forward_validation.ipynb"
...
for marker in [
    ...
    "n03_calibrated_predictions.parquet",
    "n03_e_calibrated_predictions.parquet",
    "n03_model_spec.json",
    "STRUCTURAL_FEATURES",
    "fav_deficit",
    ...
    assert marker in all_text, f"N03 marker missing: {marker!r}"
...
for forbidden in [
    ...
    assert forbidden not in all_code, f"forbidden N03 code path present: {forbidden!r}"

spec_path = RESULTS_DIR / "n03_model_spec.json"
pred_path = RESULTS_DIR / "n03_calibrated_predictions.parquet"
e_pred_path = RESULTS_DIR / "n03_e_calibrated_predictions.parquet"
deliverables_checked = False

if spec_path.exists() or pred_path.exists() or e_pred_path.exists():
    ...
    if spec.get("feature_pool_count") != 31 or "training_structure" not in spec:
        print("[warn] Existing N03 deliverables are stale pre-structural-deficit artifacts; notebook structure checked only.")
    else:
        ...
        assert spec["training_structure"]["unique_trigger_play_rows"] == 7854
        assert spec["training_structure"]["full_trigger_event_rows"] == 11416
...
print(f"[ok] N03 notebook structure valid. cells={len(cells)}")
print(f"file: {NB_PATH} ({NB_PATH.stat().st_size:,} bytes)")
if deliverables_checked:
    print("[ok] N03 deliverables present and schema-checked")
